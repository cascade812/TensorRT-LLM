...
public:
    ...
    virtual void run(AttentionOp& op, bool const is_context, int32_t const seq_offset, int32_t const num_seqs,
        ...
        std::optional<torch::Tensor> relative_attention_bias, std::optional<int64_t> compressed_kv_cache_pool_ptr) const
        = 0;
};
...
void attention(torch::Tensor q, std::optional<torch::Tensor> k, std::optional<torch::Tensor> v, torch::Tensor& output,
    ...
    bool sage_attn_qk_int8, int64_t num_contexts, int64_t num_ctx_tokens, bool const cross_attention,
    std::optional<torch::Tensor> cross_kv, std::optional<torch::Tensor> encoder_input_lengths,
    ...
    std::optional<int64_t> compressed_kv_cache_pool_ptr)
{
    ...
    if ((num_contexts > 0) && (attn_input_type != AttentionInputType::GenerationOnly))
    {
        ...
        runner->run(*op,
            ...
            flash_mla_tile_scheduler_metadata, flash_mla_num_splits, cross_attention, cross_kv, encoder_input_lengths,
            relative_attention_bias, compressed_kv_cache_pool_ptr);
    }
